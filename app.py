...
def restaurants_by_zip(zipcode):
    try:
        headers = {'User-Agent': 'MealMatesApp/1.0 (jr6577@nyu.edu)'}
        resp = requests.get(
        f'https://nominatim.openstreetmap.org/search?postalcode={zipcode}&country=USA&format=json',
        headers=headers
        )
        resp.raise_for_status()  # Raise an exception for HTTP errors (4xx or 5xx)
        geocoding_data = resp.json()
        ...
        return jsonify({'error': 'Failed to decode location data'}), 500
    except KeyError:
        print(f"KeyError: Could not find expected data in geocoding response: {resp.json()}")
        return jsonify({'restaurants': [], 'center': {'lat': 0, 'lon': 0}, 'warning': 'Could not determine precise location'}), 200
