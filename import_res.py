...
with engine.connect() as conn:
    df_res = pd.read_sql(text("SELECT name, address, latitude, longitude, score, cuisine FROM restaurants"), conn)
...
with app.app_context():
    ...
    for _, row in df_res.iterrows():
        ...
        if pd.notna(cuisine):
            cuisine = str(cuisine).lower().replace("/", ",").replace("  ", " ").strip()
        else:
            cuisine = None
...
print(f"Imported {len(df_res)} restaurants into local db")
